...
function dragula (initialContainers, options) {
  ...
  //   var datas = evt.data.split('|');
  //   // console.log(document.getElementById(datas[0]));
  //   // console.log(document.getElementById(datas[1]));
  //   drop(document.getElementById(datas[0]),document.getElementById(datas[1]));
  // }
  ...
  function onMessage(evt) {
    ...
    console.log('**************');
    var i = document.getElementById(datas[0]);
    var d = document.getElementById(datas[1]);
    var s = document.getElementById(datas[2]);
    var c = i.cloneNode(true);
    var cs = document.getElementById(datas[3]);

    // _copy = document.getElementById(datas[4]);
    _item = i;
    drake.emit('cloned', c, i, 'copy');
    // console.log(i);
    // console.log('i');
    // console.log(d);
    // console.log('d')
    ...
    var parent = getParent(i);
    // s.removeChild(_item); -> 이건 틀림
    // d.appendChild(_item); -> 대기 이방법이 맞는듯
    parent.removeChild(_item);
    drop(i,d);
    d.appendChild(i);
    i.className = i.parentNode.id;
    // drake.emit('drop', i, d, s, cs);
    // console.log('drop!!');
    // cleanupSocket(i);

    // alert('!!');
    
    // doDrop(document.getElementById(datas[0]),document.getElementById(datas[1]));
  }
...
// ws.onclose = onClose;
  ...
  function release (e) {
    ...
    if (dropTarget && ((_copy && o.copySortSource) || (!_copy || dropTarget !== _source))) {
      // console.log(_source);
      if(item.className === item.parentNode.id + " gu-transit" || item.className === item.parentNode.id) {
        dropSamePlace(item,dropTarget);
      }else {
        ...
        // console.log(item);
        // console.log(String(document.getElementById(dropTarget.id)));
        ...
        if(_currentSibling === null) {
          ws.send(item.id+"|"+ dropTarget.id +"|"+ _source.id + "|" + null);
        }else {
        ws.send(item.id+"|"+ dropTarget.id +"|"+ _source.id + "|" + _currentSibling.id);
        }
        // ws.onmessage = onMessage;
  ...
  function dropSamePlace (item, target) {
  ...
  function doDrop (item, target) {
     ...
     drake.emit('drop', item, target, _source, _currentSibling);
    //  if (isSuccessSendInitialPlacement(target,item)) {
    //    drake.emit('cancel', item, _source, _source);
    //  } else {
    //    drake.emit('drop', item, target, _source, _currentSibling);
    //  }
  ...
  function drop (item, target) {
    ...
    //   parent.removeChild(_item);
    // }
    // if (isInitialPlacement(target,item)) {
    //   console.log("success");
    //   drake.emit('cancel', item, _source, _source);
    // } else {
    //   console.log('fail');
  ...
  function isSuccessSendInitialPlacement (target,item,s) {
    // console.log(item);
    var sibling;
    if (s !== void 0) {
      sibling = s;
